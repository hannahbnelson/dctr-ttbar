...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def process(self, events):     
        ...
        # df['weights']=np.ones_like(events['event'])[event_selection_mask]
        # # df['mtt']=(tops[:,0] + tops[:,1]).mass
        # # df['mll']=mll
        # # df['tops_pt']=tops.sum().pt
        # # df['avg_top_pt']=np.divide(tops.sum().pt, 2.0)
        # # df['njets']=njets

        # tops["top1pt"] = tops.pt[:,0]
        # # df['top1pt']=tops.pt[:,0]
        ...
        variables_to_fill = {
            ...
            "avg_top_pt": np.divide(tops.sum().pt, 2.0),
            "mtt"       : (tops[:,0] + tops[:,1]).mass,
            "top1pt"    : tops.pt[:,0],
            "top1eta"   : tops.eta[:,0],
            "top1phi"   : tops.phi[:,0],
            "top1mass"  : tops.mass[:,0],
            "top2pt"    : tops.pt[:,1],
            "top2eta"   : tops.eta[:,1],
            "top2phi"   : tops.phi[:,1],
            "top2mass"  : tops.mass[:,1],
        }
