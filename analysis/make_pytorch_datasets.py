...
def main(fsmeft_train, fpowheg_train, fsmeft_val, fpowheg_val, outdir, title):
    ...
    inputs_powheg_train = pickle.load(gzip.open(fpowheg_train)).get().query('weights>0') # only use non negative powheg events (~0.4% events are negative)
    inputs_powheg_val = pickle.load(gzip.open(fpowheg_val)).get().query('weights>0') # only use non negative powheg events (~0.4% events are negative)
    ...
    with open(yaml_path, 'w') as file: 
        yaml.safe_dump({'means': means.to_dict(), 'stdvs': stdvs.to_dict(),}, file)
    print(f"standardization constants saved to {yaml_path}")

    inputs_smeft_train = DNN_tools.standardize_df(inputs_smeft_train.drop(['weights'], axis=1), means, stdvs).to_numpy().astype(np.float32)
    inputs_smeft_val = DNN_tools.standardize_df(inputs_smeft_val.drop(['weights'], axis=1), means, stdvs).to_numpy().astype(np.float32)
    inputs_powheg_train = DNN_tools.standardize_df(inputs_powheg_train.drop(['weights'], axis=1), means, stdvs).to_numpy().astype(np.float32)
    inputs_powheg_val = DNN_tools.standardize_df(inputs_powheg_val.drop(['weights'], axis=1), means, stdvs).to_numpy().astype(np.float32)

    np.save(f"{title}_inputs_smeft_train.npy", inputs_smeft_train)
    print(f"smeft training dataset saved to: {title}_inputs_smeft_train.npy")

    np.save(f"{title}_inputs_smeft_val.npy", inputs_smeft_val)
    print(f"smeft validation dataset saved to: {title}_inputs_smeft_val.npy")

    np.save(f"{title}_inputs_powheg_train.npy", inputs_powheg_train)
    print(f"powheg training dataset saved to: {title}_inputs_powheg_train.npy")

    np.save(f"{title}_inputs_powheg_val.npy", inputs_powheg_val)
    print(f"powheg validation dataset saved to: {title}_inputs_powheg_val.npy")
...
if __name__=="__main__":
    ...
    parser.add_argument('--title', default='numpy', help='text to add at the beginning of pkl files')
